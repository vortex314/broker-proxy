...
template <class T> class SinkFunction : public Sink<T> {
...
template <typename T> Flow<T, T> &filter(std::function<bool(const T &t)> f) {
...
//
template <class IN, class OUT>
Source<OUT> &operator>>(Source<OUT> &publisher, Flow<IN, OUT> &flow) {
...
public:
  ...
  template <class T> Source<T> &operator>>(Source<T> &source) {
    _requestables.push_back(&source);
    return source;
  }
